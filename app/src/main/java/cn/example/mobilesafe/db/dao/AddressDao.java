...
public class AddressDao {
    ...
    private static final String PATH = "/data/data/cn.example.application.mobilesafe/files/address.db";// 注意,该路径必须是data/data目录的文件,否则数据库访问不到
        ...
        if (number.matches("^1[3-8]\\d{9}$")) {
            ...
            switch (number.length()) {
                ...
                case 8:
                    ...
                    break;
                default:
                    if (number.startsWith("0") && number.length() > 10) {//有可能是长途电话,有些区号是4位,有些区号是3位(包括0)
                        //先查4位
